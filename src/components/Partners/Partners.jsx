...
// You can add more logos here in the same format
const partnerLogos = [
...
const Partners = () => {
  ...
  const visibleLogos = partnerLogos.slice(currentIndex, currentIndex + LOGOS_PER_SET);

  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex(prevIndex =>
        (prevIndex + LOGOS_PER_SET) % partnerLogos.length
      );
    }, INTERVAL_MS);
    ...
    <section className="section-padding animate-fade-in-up" style={{ marginTop: "-40px" }}>
      <div className="container mx-auto px-4">
        ...
        <div className="hidden md:block">
          <div className="partner-logos py-6">
            <div className="grid grid-cols-4 md:grid-cols-4 gap-y-6 gap-x-4 transition-all duration-500">
              {visibleLogos.map((partner, index) => (
                <div
                  key={index}
                  className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 flex items-center justify-center transition-all hover:shadow-md hover:-translate-y-1 duration-300"
                >
                  <img
                    src={partner.logo}
